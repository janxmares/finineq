# Preliminary analysis, top10 and top1 shares
# Jan Mares, 191203
...
data_dm[, c("year","year_2000","WarYears","RevCoups","Top1share","GiniNet") := NULL]

data_dm <- data_dm[complete.cases(data_dm), ]
dummies <- colnames(dum)[2:(ncol(dum))]

# run BMA
bma_1y_top10_baseline <- bms(data_dm, iter=5000000, burn=1000000, mprior = "uniform", g = "hyper",
                    nmodel=5000, mcmc="bd.int",
                    fixed.reg = dummies, user.int = F)

coef(bma_1y_top10_baseline, exact = T)
summary(bma_1y_top10_baseline)

# save the results into file
save(bma_1y_top10_baseline, file = here("Results/bma_1y_top10_baseline.Rdata"))
...
coef(bma_5y_top10_baseline, exact = T, std.coefs = T)
...
dum <- dummy(data_dm$year, sep = "_")
data_dm <- cbind(data_dm, dum)
data_dm[, c("year","year_2000","WarYears","RevCoups","Top10share","GiniNet") := NULL]

data_dm <- data_dm[complete.cases(data_dm), ]
dummies <- colnames(dum)[2:(ncol(dum))]

# run BMA
bma_1y_top1_baseline <- bms(data_dm, iter=5000000, burn=1000000, mprior = "uniform", g = "hyper",
                    nmodel=5000, mcmc="bd.int",
                    fixed.reg = dummies, user.int = F)

coef(bma_1y_top1_baseline, exact = T)
summary(bma_1y_top1_baseline)

# save the results into file
save(bma_1y_top1_baseline, file = here("Results/bma_1y_top1_baseline.Rdata"))
...
coef(bma_5y_top1_baseline, exact = T, std.coefs = T)
