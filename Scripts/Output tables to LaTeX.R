...
explvarshort <- c("NatRes","GovExp","GDSavings","PopGrowth","PopTot","Infl","EducExp",
                  "VAI","VAA","NetFDIin","RuleofLaw","Mortality","GDPpc","GDPgrowth","TradeOpen",
...
explvarnames <- c("Natural resources rents","Government expenditures","Gross domestic savings",
                  ...
                  "Rule of law","Mortality","GDP per capita", "GDP growth","Trade openness",
                  "Life expectancy","Unemployment","Equipment investment","Non-equipment investment",
                  "Chinn-Ito index","Financial institutions depth","Access to financial institutions",
                  "Financial institutions efficiency", "Financial markets depth", "Economic freedom",
                  "Education index (UN)","Civil liberties & political rights","Left-wing orientation",
                  "Redistribution","Financial globalization","Restrictions on globalization",
...
# results_all <- coef(bma_1y_top10_baseline, exact=T, std.coefs = T)

# results_all <- coef(bma_5y_top10_baseline, exact=T, std.coefs = T)
# results_all <- coef(bma_1y_top1_baseline, exact=T, std.coefs = T)
# results_all <- coef(bma_5y_top1_baseline, exact=T, std.coefs = T)
